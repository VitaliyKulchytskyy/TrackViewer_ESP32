...
uint32_t updateTime = 0;       // time for next newLabelUpdate
...
void setup(void) {
  ...
  updateTime = millis(); // Next newLabelUpdate time
}


void loop() {
  if (updateTime <= millis()) {
    updateTime = millis() + 25; // Delay to limit speed of newLabelUpdate
    ...
    //Serial.println(millis()-t); // Print time taken for meter newLabelUpdate
  }
}
...
void analogMeter()
{
  ...
  tft.setTextColor(TFT_BLACK);  // Label colour
...
void plotNeedle(int value, byte ms_delay)
{
  ...
  while (!(value == old_analog)) {
    ...
    // Wait before next newLabelUpdate
    delay(ms_delay);
  }
}
